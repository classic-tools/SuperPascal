...
function  GetMicroSecondTime: MicroSecondTimeType;
    attribute (name = '_p_GetMicroSecondTime'); external;
...
begin
  { A nonstandard function reads
    the processor time in microseconds }
  t := integer((GetMicroSecondTime()+500) div 1000)
end;
...
begin
  if ParamCount >= 1 then
    copyparam(codename, 1)
  else
    begin
      write('    code = ');
      readphrase(codename);
    end;
  trimfilename(codename);
  if ParamCount = 1 then select := false
