...
{ convert space padding in filenames to nulls for posix systems }
procedure trimfilename(var name: phrase);
var n : integer;
    done : boolean;
begin
    done := false;
    for n := maxphrase downto 2 do
        if name[n] <> sp
        then done := true
        else if not done 
        then name[n] := chr(null)
end;

procedure copyparam(var name: phrase; n: integer);
begin
    if maxphrase >= length(ParamStr(n)) then name := ParamStr(n)
    else begin
        writeln(StdErr, 'Parameter: ', n, ' is too large.');
        writeln(StdErr, '(max length is ', maxphrase, ' characters.)');
        name := SubStr(ParamStr(n), 1, maxphrase)
    end
end;
